...
public class SessionRepository {
    ...
    public void save(String token, UserIdentity user, Duration ttl) {
        String sessionKey = SESSION_KEY + token;
        String userSessionsKey = USER_SESSIONS_KEY + user.id();
    ...
    public void remove(String token) {
        ...
        if (json != null) {
            ...
            if (user != null) {
                redis.opsForSet().remove(USER_SESSIONS_KEY + user.id(), token);
            }
        }
    }
    ...
    public void removeAllByUserId(Long userId) {
        ...
        if (tokens != null) {
            tokens.forEach(t -> redis.delete(SESSION_KEY + t));
        }
